...
void find_girth_inducing_cycles(size_t p, size_t s_limit) {
            ...
            pool.do_job(
                [&, quadratics_in_job, start_quad_id] {
                    ...
                    for (auto& [g, subsets] : local_girths) {
                        girths[g].insert(girths[g].end(), subsets.begin(), subsets.end());
                    }
    ...
    //         std::cout << "g: " << g << ", a: " << subset.front() << std::endl;
    //     }
    // }
    ...
    for (; s <= quadratics.size() && s < s_limit; s++) {
            ...
            if (combinations_in_job > 0) pool.do_job(
                ...
                [&, start_combination_id, combinations_in_job] {
                    ...
                    for (unsigned long combination_id = 0; combination_id < combinations_in_job; combination_id++, std::next_permutation(combination_indices.rbegin(), combination_indices.rend())) {
                        ...
                        // TODO: subsets will often induce a girth for a range of combination ids: maybe we can analyze the subsets and form some kind of sieve algorithm to skip combinations that have known subsets in them?
        ...
        for (size_t a = 0; a < p; a++) {
            ...
            if ((singles[a] == s + 1 || !coefficients_to_keep.contains(a)) && quadratics.erase(a) > 0) { // or g < 3?
                // I THINK THIS WORKS?
                std::erase(quadratic_coefficients, a);
                for (auto& [g, subsets] : girths) {
                    std::erase_if(
...
int main(int, char**) {
    ...
    //     std::cout << std::endl;
    // }
    find_girth_inducing_cycles(1109, 1109);
